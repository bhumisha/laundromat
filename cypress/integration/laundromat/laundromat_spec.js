...
describe('tests laundromat signup functionality', () => {
  ...
  it('verifies new laundromat can login in and does not see any orders yet and can logout', () => {
    loginAsLaundromat(laundromat);
    cy.contains('No orders yet!');

    cy.get('.navbar-nav > #loginBtn').should('not.exist');
    cy.get('#adminLogout').click();
    cy.get('.navbar-nav > #loginBtn').should('exist');
  });
});
